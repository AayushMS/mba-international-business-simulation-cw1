# Mobilé Inc. Strategic Analysis Project

## Project Overview
MBA coursework (MN7002NI) — International Business Strategy with Simulation. Building an interactive React+Vite website as a research & prep tool for a 30-slide group presentation analyzing Mobilé Inc.'s strategic position in 2025.

**Deadline:** 10 April 2026 | **Weight:** 30% of module grade | **Format:** PDF of max 30 slides

## The Case
Mobilé Inc. is a US smartphone manufacturer competing in USA, Europe, and Asia. Post-DOJ intervention (early 2025) ending tacit price coordination, the company faces three simultaneous technology waves:
- **Wave 1 (4G LTE):** Mature cash engine, ~$4.8B revenue, under price pressure
- **Wave 2 (5G):** Urgent — 70%+ urban coverage, Mobilé has NOT launched a 5G handset yet
- **Wave 3 (AI-devices):** 2027-28 horizon, R&D investment decisions must happen NOW

**Central constraint:** USD 90M minimum cash reserve floor. All three waves compete for the same finite resource pool.

## Team Members
| # | Name | College ID | University ID |
|---|------|-----------|---------------|
| 1 | Bishwesh Ram Shrestha | np01mb7a250082 | 25030247 |
| 2 | Gaurav Dangol | np01mb7a250083 | 18029811 |
| 3 | Bishan Subedi | np01mb7a250010 | 25030193 |
| 4 | Aayush Man Singh | np01mb7a250043 | 18029779 |
| 5 | Ruchan Jung Sijapati | NP01MB7A250143 | 20048645 |
| 6 | Shaswat Nibha Maharjan | np01mb7a250162 | — |

## Critical Rules for All Agents

### Content Rules
1. **CASE-SPECIFIC ONLY.** Every analytical point must trace to specific Mobilé Inc. data from the case study. Generic statements like "to be the leading smartphone company globally" score zero.
2. **THREE-WAVE COHERENCE.** Every section must acknowledge the simultaneous 4G/5G/AI tension. This is THE defining challenge of the case.
3. **TASKS BUILD ON EACH OTHER.** Task 3 (SWOT) must only contain evidence from Task 2. Task 4 must align with Tasks 1-3. No contradictions between tasks.
4. **NO NEW INFORMATION IN SWOT.** Every SWOT point must trace to Task 2 findings. No unsupported claims.
5. **HARVARD REFERENCING.** All citations must use Harvard format.
6. **EXECUTIVE COMMUNICATION.** Task 4 recommendation must be clear, specific, actionable — not a summary of analysis.

### Marking Priorities (by weight)
- Internal & External Analysis: **30 marks** — deepest analysis required, VRIO/RBV, Value Chain, PESTLE, Porter's Five Forces, Strategic Group, CSF
- Strategy Process: **15 marks** — vision, mission, goals, SMART objectives
- SWOT Synthesis: **15 marks** — evidence-based, case-specific, strategically interpreted
- Strategic Recommendation: **15 marks** — clear strategic choice with trade-off honesty
- Presentation Structure: **10 marks** — professional quality, originality
- Individual Reflection: **10 marks** — 700+ words per person
- Executive Summary: **5 marks** — distill all 4 tasks into one cohesive slide

### What Gets Distinction (85%+)
- Outstanding command of international business strategy applied specifically to Mobilé Inc.
- Three-wave structure fully understood and integrated throughout every task
- Exceptional synthesis across multiple analytical frameworks with genuine strategic insight
- Recommendations fully specific, differentiated by market and technology wave
- Trade-offs addressed honestly (including $90M cash floor constraint)
- Comprehensive, error-free Harvard citations

## Agent Team: `mobile-inc-strategy`

### Team Roles & Skill Assignments

| Agent | Type | Role | Skills to Use |
|-------|------|------|---------------|
| **team-lead** | planner | Orchestrates work, manages dependencies, final coherence review | `marking-criteria-reviewer` — for final quality gate before completion |
| **researcher** | researcher | Deep case study analysis, builds/validates knowledge base, applies strategic frameworks | `mobile-inc-researcher` — on EVERY research output to ensure case-specificity; `strategic-framework-analyst` — when building or validating any framework analysis (VRIO, PESTLE, Porter's, Value Chain, Strategic Group, CSF, SWOT) |
| **frontend-dev** | designer | Builds React+Vite website with interactive components | No skills required — follows component specs in design doc |
| **content-writer** | deep-executor | Writes strategic analysis content for Tasks 1-4, executive summary | `mobile-inc-researcher` — on EVERY piece of content to ensure case-specificity and three-wave coherence; `strategic-framework-analyst` — when writing Task 2 (Internal & External Analysis) and Task 3 (SWOT Synthesis) content |
| **quality-reviewer** | code-reviewer | Reviews all output against marking criteria, flags gaps | `marking-criteria-reviewer` — this IS the quality reviewer's primary skill, use it on every piece of content reviewed |

### When Each Skill Triggers

#### `mobile-inc-researcher` (path: `skills/mobile-inc-researcher/SKILL.md`)
**Used by:** researcher, content-writer
**When to invoke:**
- Before writing ANY analytical content — ensures case-specificity
- When extracting or citing case study data — ensures accuracy
- When building framework analyses — enforces three-wave lens
- When writing Task 1-4 content — flags generic statements
- When connecting findings across tasks — ensures cross-task consistency

#### `strategic-framework-analyst` (path: `skills/strategic-framework-analyst/SKILL.md`)
**Used by:** researcher, content-writer
**When to invoke:**
- When building or validating VRIO, Value Chain, PESTLE, Porter's Five Forces, Strategic Group, or CSF analysis
- When writing SWOT synthesis — ensures every point traces to framework evidence
- When checking internal-external connections between frameworks
- When justifying framework selection ("Why this framework for Mobilé Inc.?")
- During Task 2 and Task 3 work specifically

#### `marking-criteria-reviewer` (path: `skills/marking-criteria-reviewer/SKILL.md`)
**Used by:** quality-reviewer, team-lead
**When to invoke:**
- After EACH task's content is written — quality gate before moving to next task
- During Phase 3 quality review — comprehensive check of all content
- During Phase 4 final integration — team-lead's coherence check
- Before declaring any task "complete" — ensures distinction-level quality
- When checking cross-task consistency (Task 3 vs Task 2, Task 4 vs Tasks 1-3)

## Project Structure
- `knowledge-base/` — All case data and framework analyses in markdown
- `knowledge-base/frameworks/` — Individual framework analyses (VRIO, PESTLE, etc.)
- `website/` — React+Vite interactive website
- `skills/` — Custom Claude Code skills (3 skills, see above for assignments)
- `docs/superpowers/specs/` — Design specifications

## Key Data Points (Quick Reference)
- Revenue: ~$4.8B annually (4G)
- Market share: 25% each (4 firms, all markets)
- Cash reserve floor: $90M
- Borrowing: 4.8% p.a. normal, 7.8% emergency
- Atlanta plant: 12 lines, $2.9M/line, $28/hr labor
- Vietnam plant: growing, $3.3M/line, $6/hr labor
- USA market: ~$80B, 14-20% growth, 88% smartphone penetration
- Europe: 450M users, 55% 5G coverage, 16-22% growth
- Asia: ~1B users, bifurcated (premium 5G + mass 4G), 22-30% growth
- 5G adoption: 38% US, 28% EU by end-2024
- AI-device commercial availability: 2027-28
- New plant lead time: 2 years from investment to operational
- Overproduction penalty: 5-10% cost; Underproduction = permanently lost sales
